(function ($) {
	...
	$.fn.shareButtons = function (url, options) {
		...
		var shareURLs = {
			...
			'facebook' : 'https://facebook.com/sharer.php?p[url]=' + url_encoded,
		...
		// Facebook supports:
		// options.facebook.image - the thumbnail to use.

		if(options.facebook) {

			if (options.facebook.image) {
				shareURLs['facebook'] += '&p[images][0]=' + encodeURIComponent(options.facebook.image);
			}
